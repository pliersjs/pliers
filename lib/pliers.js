...
module.exports = function pliers(options) {
  ...
  function addFileset(id, includePatterns, excludePatterns) {
    ...
    function globPattern() {
      ...
      return _.uniq(filesets[id].sort(), true)
    }
    ...
    Object.keys(patterns).forEach(function (group) {
      ...
      if (Array.isArray(values[group])) {
        values[group] = _.uniq(values[group])
      } else {
  ...
  function run(name, cb) {
    ...
    var time
    if (options.duration) time = Date.now()
    ...
    fnTasks.push(function (cb) {
      if (options.duration) {
        ...
        options.logger.info('Completed task: [' + duration + 'ms] ' + name)
      } else {
  ...
  function watch(files, fn) {
    ...
    function watchFile(file) {
      ...
      var fsWatcher = fs.watch(file,  { persistent: true }, function (event) {
        ...
        options.logger.debug('Changed: ' + relative(options.cwd, file) + ' (' + event + ')')
