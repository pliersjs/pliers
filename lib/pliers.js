...
module.exports = function pliers(options) {
  ...
  function run(name, cb) {
    ...
    taskDeps.forEach(function (dep) {
      if (typeof dep === 'function') {
        ...
        foundTask = getTask(dep)
        if (foundTask) {
          fnTaskDeps = fnTaskDeps.concat(foundTask)
        } else {
    ...
    async.series(fnTaskDeps, cb || noop)
  }
  ...
  function load(path) {
    var sub = pliers(_.extend({}, options, { c: path }))
      , configPath
